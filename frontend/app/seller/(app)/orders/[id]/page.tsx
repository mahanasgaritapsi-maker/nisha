...
export default function SellerOrderDetailPage({ params }: PageProps) {
  ...
  if (error || !data) {
    return (
      <div className="space-y-4">
        ...
        <Link href={paths.seller.orders} className="text-sm text-indigo-600 hover:underline">
          Back to orders
        </Link>
      </div>
    );
  ...
  const confirmTitle =
    pendingAction === "reject_payment" ? "Reject payment" : "Cancel order";
  const confirmMessage =
    pendingAction === "reject_payment"
      ? "Stock will be restored and the customer must pay again."
      : "This will cancel the order and restore stock if applicable.";
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="pt-0">
          ...
          <div className="mt-4 flex justify-end gap-6 text-sm">
            <span className="text-neutral-500">
              Subtotal: {formatMoney(data.subtotal_amount)}
            </span>
              ...
              {data.payment_proofs.map((proof) => (
                <div key={proof.id} className="space-y-2">
                  ...
                  <p className="text-xs text-neutral-500">
                    Uploaded {formatDateTime(proof.uploaded_at)}
                  </p>
                </div>
              ))}
            </div>
      ...
      <ConfirmModal
        ...
        confirmLabel={pendingAction ? ORDER_ACTION_LABELS[pendingAction] : "Confirm"}
        loading={actionLoading}
